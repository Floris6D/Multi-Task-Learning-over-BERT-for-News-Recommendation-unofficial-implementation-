...
class NewsEncoder(torch.nn.Module):
    ...
    def forward(self, tokens, cat_tokens, mask = False, cat_mask = False):
        ...
        bs, n, ts = cat_tokens.shape
        cat_tokens = cat_tokens.reshape(bs*n, ts)
        cat_mask = cat_mask.reshape(bs*n, ts)
        c_x = self.bert(cat_tokens, cat_mask)
        cat = self.forward_cat(c_x.last_hidden_state)
